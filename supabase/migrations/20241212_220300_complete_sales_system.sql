-- =====================================================
-- MIGRACIÓN: Completar sistema de ventas
-- =====================================================
-- Fecha: 2024-12-12
-- Descripción: Completar lo que falta del sistema de ventas
-- =====================================================

-- Verificar si la tabla sale_items existe, si no, crearla
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        -- Crear tabla para detalles de venta
...
DO $$
BEGIN
    -- Política para vendedores ver sus propios sale_items
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sale_items' 
        AND policyname = 'Vendedores pueden ver sus propios sale_items'
    ) THEN
    ...
    -- Política para vendedores insertar sale_items
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sale_items' 
        AND policyname = 'Vendedores pueden insertar sale_items'
    ) THEN
    ...
    -- Política para jefes ver todos los sale_items
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'sale_items' 
        AND policyname = 'Jefes pueden ver todos los sale_items'
    ) THEN
...
-- Agregar columna custom_prices a customers si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ADD COLUMN custom_prices JSONB DEFAULT '{}';
    END IF;
END $$;

-- Crear funciones solo si no existen
...
$$ LANGUAGE plpgsql;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.routines 
        WHERE routine_name = 'upsert_customer'
        AND routine_schema = 'public'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.routines 
        WHERE routine_name = 'get_vendor_sales_with_items'
        AND routine_schema = 'public'
    ) THEN
...
BEGIN
    ...
    RAISE NOTICE 'Migración completada exitosamente: Sistema de ventas completo';
END $$;
