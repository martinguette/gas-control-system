...
export function ExchangeFlow({ onCalculatedItems }: ExchangeFlowProps) {
  const [emptyCylinders, setEmptyCylinders] = useState<EmptyCylinder[]>([]);
  const [calculatedItems, setCalculatedItems] = useState<any[]>([]);
  const { control, setValue, watch } = useFormContext<SaleFormData>();
  ...
  // Función para actualizar un cilindro vacío
  const updateEmptyCylinder = (
    id: string,
    field: keyof EmptyCylinder,
    value: any
  ) => {
    setEmptyCylinders((prev) =>
      prev.map((cylinder) =>
        cylinder.id === id ? { ...cylinder, [field]: value } : cylinder
      )
    );
  };
      ...
      <Card className="border-blue-200 bg-blue-50">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-blue-600">
            Registra todos los cilindros vacíos que el cliente te entrega
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {emptyCylinders.length === 0 ? (
            <div className="text-center py-6 text-blue-600">
              ...
              <p className="text-sm">No hay cilindros vacíos registrados</p>
              <p className="text-xs text-blue-500">
                Haz clic en "Agregar Vacío" para comenzar
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {emptyCylinders.map((cylinder, index) => (
                <Card
                  key={cylinder.id}
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                    <div>
                      <FormLabel className="text-xs text-blue-700">
                        Marca
                      </FormLabel>
                      <Select
                        value={cylinder.brand}
                        onValueChange={(value) =>
                          updateEmptyCylinder(cylinder.id, 'brand', value)
                        }
                      >
                    ...
                    <div>
                      ...
                      <Select
                        value={cylinder.type}
                        onValueChange={(value) =>
                          updateEmptyCylinder(cylinder.id, 'type', value)
                        }
                      >
                        <SelectTrigger className="text-sm border-blue-200">
                          <SelectValue />
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        ...
                        <div className="h-10 px-3 py-2 text-sm border border-blue-200 rounded-md bg-blue-50 flex items-center">
                          <div
                      ...
                      <Button
                        ...
                        className="h-10 w-10 p-0 text-red-600 hover:text-red-700 border-red-200"
                      >
            ...
            <div className="bg-blue-100 p-3 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-blue-800">
                  Resumen de Vacíos Recibidos
                </span>
              </div>
              <div className="text-sm text-blue-700">
                <p>
                  Total de cilindros vacíos:{' '}
                  <strong>{getTotalEmptyCylinders()}</strong>
                </p>
                <div className="mt-1 space-y-1">
                  {Object.entries(
                    emptyCylinders.reduce((acc, cylinder) => {
                      const key = `${cylinder.brand} ${cylinder.type}`;
                      if (!acc[key]) acc[key] = 0;
                      acc[key] += cylinder.quantity;
                      return acc;
                    }, {} as Record<string, number>)
                  ).map(([key, quantity]) => (
